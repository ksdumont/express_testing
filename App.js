...
app.get('/sum', (req, res) => {
    ...
    if (!a) {
        return res
        .status(400).send('Value for a is needed');
    }
    if (!b) {
        return res
        .status(400).send('Value for b is needed');
    }
    ...
    if (isNaN(numA)) {
        return res
        .status(400).send('Value for a must be numeric');
    }
    if (isNaN(numB)) {
        return res
        .status(400).send('Value for b must be numeric');
    }
    if (numB == 0) {
        return res
        .status(400).send('Cannot divide by 0');
    }
    const ans = numA / numB;
...
app.get('/frequency', (req, res) => {
    ...
    const counts = s.toLowerCase().split('')
        .reduce((acc, curr) => {
            if (acc[curr]) {
                acc[curr] ++;
            } else {
